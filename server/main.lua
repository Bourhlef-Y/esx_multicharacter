...
local DATABASE do
	...
	AddEventHandler('playerConnecting', function(playerName, setKickReason, deferrals)
		...
		if identifier then
			...
			if not ESX.GetConfig().EnableDebug then
				if ESX.Players[identifier] and not Config.AllowMultipleConnections then
	...
	MySQL.ready(function()
		...
		if DB_COLUMNS then
			...
			-- Check if skin column exists before including it in query
			local skinColumnExists = MySQL.scalar.await("SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'users' AND COLUMN_NAME = 'skin'", {DATABASE})
			
			if skinColumnExists and skinColumnExists > 0 then
	...
	RegisterNetEvent('esx_multicharacter:changepersonnage', function()
		local source = source
		TriggerEvent('esx:playerLogout', source)
	end)
